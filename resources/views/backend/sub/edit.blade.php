...
@section('title', 'Edit Sub-Kriteria | SPK-AHP')
{{-- @endsection --}}
@section('content')
    ...
    <!-- Judul Halaman -->
        ...
        <div class="card">
            <div class="card-body">
                ...
                @endif
                @foreach ($sb as $val)
                <form class="needs-validation" method="POST" action="/editsb/{{ $val->id }}">
                    @csrf
                    <div class="form-group">
                        ...
                        <select class="custom-select @error('id_kriteria') is-invalid @enderror" name="id_kriteria" id="id_kriteria">
                            ...
                            <option value="{{ $item->id }}" {{ $item->id == $val->id_kriteria ? 'selected' : ''}}>{{ $item->nama_kriteria }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control form-control-user @error('nama_sub') is-invalid @enderror" id="nama_sub" placeholder="Masukkan Nama Kriteria"
                            name="nama_sub" value="{{ $val->nama_sub }}">
